...
async def get_overview(links, mbid=None):
    ...
    overview_providers = [p for p in provider.get_providers_implementing(provider.ArtistOverviewMixin) if p.is_valid()]
...
async def get_artist_info_multi(mbids):
    ...
    artist_art_providers = [p for p in provider.get_providers_implementing(provider.ArtistArtworkMixin) if p.is_valid()]
...
async def get_release_group_info_multi(mbids):
    ...
    album_art_providers = [p for p in provider.get_providers_implementing(provider.AlbumArtworkMixin) if p.is_valid()]
